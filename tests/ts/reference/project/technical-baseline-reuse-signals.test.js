...
function allReadFields(request) {
  const rootGroups = request.requestReadPlan?.groups ?? [];
  const legacyGroups = request.agentAction?.read?.fieldGroups ?? [];
  return [...rootGroups, ...legacyGroups].flatMap((group) => group.fields ?? []);
}
