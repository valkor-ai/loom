...
function summaryForInstruction(mode: string, instruction: Record<string, unknown>): string {
  ...
  if (mode === "execute_task") {
    ...
    return withNoRecapGuard(`ACTION REQUIRED: execute_task is auto-runnable. Read ${requestRef}, use requestReadPlan.groups inspect readCommands, complete the task, write the resultFile, and run submitCommand now. A final response before resultFile exists and submitCommand succeeds is invalid. Do not summarize progress or ask whether to continue.`);
  }
  if (mode === "run_cli") {
  ...
  if (mode === "generate_candidate") {
    ...
    if (instruction.candidateKind === "ArchitectureSections" && instruction.sectionGenerationMode === "single_section") {
      ...
      return withNoRecapGuard(`ACTION REQUIRED: generate_candidate is auto-runnable. Read ${requestRef}, use requestReadPlan.groups inspect readCommands, write only the ${targetSection} section to ${targetCandidateFile}, then run instruction.completionBarrier.followUpCommand.commandInvocation as the next agent tool action so the CLI can route the next missing section or submit_existing_candidate. This follow-up command is not a user instruction. Do not summarize progress or ask whether to continue.`);
    }
    if (instruction.candidateKind === "TaskPlanGroupedOutputs") {
      ...
      return withNoRecapGuard(`ACTION REQUIRED: generate_candidate is auto-runnable. Read ${requestRef}, use requestReadPlan.groups inspect readCommands, write TaskPlan outline to ${outlineFile}, write group files using ${groupFilePattern}, and run submitCommand now. Do not summarize request creation, report progress, or ask whether to continue.`);
    }
    return withNoRecapGuard(`ACTION REQUIRED: generate_candidate is auto-runnable. Read ${requestRef}, use requestReadPlan.groups inspect readCommands, write the requested candidate files, and run submitCommand now. Do not summarize progress or ask whether to continue.`);
  }
  if (mode === "submit_existing_candidate") {
    const requestRef = typeof instruction.requestRef === "string" ? instruction.requestRef : "instruction.requestRef";
    return withNoRecapGuard(`ACTION REQUIRED: submit_existing_candidate is auto-runnable. Read ${requestRef}, use requestReadPlan.groups inspect readCommands when request fields are needed, and run submitCommand now. Do not summarize progress or ask whether to continue.`);
  }
  if (typeof userVisibleMeaning?.summary === "string" && userVisibleMeaning.summary.length > 0) {
    return withNoRecapGuard(`ACTION REQUIRED: ${userVisibleMeaning.summary}`);
  }
