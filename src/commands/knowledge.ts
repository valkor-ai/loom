...
function knowledgeSemanticInstruction(input: {
  ...
  return withAutoRunnableTransition({
    ...
    instructions: [
      ...
      "For each chunk, decide summary, semanticLabels, and blockAffinity from the chunk meaning after reading it. Do not use regex rules, keyword tables, filename heuristics, or script-generated label/affinity factories as semantic judgment.",
      "Scripts may only serialize the already-decided JSON result into instruction.resultFile; scripts must not replace the semantic reading and judgment step.",
    ...
    forbiddenStops: [
      ...
      "do not use regex rules, keyword tables, filename heuristics, or generated script logic to decide semantic summaries, labels, or blockAffinity",
    ],
    stopOnlyWhen: [
