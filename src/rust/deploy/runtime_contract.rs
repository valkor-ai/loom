...
pub fn load_runtime_contract(project_root: &Path) -> StateResult<DeploymentRuntimeContract> {
    ...
    let delivery_id = status
        .active_delivery_id
        .or(status.last_completed_delivery_id)
        .ok_or_else(|| {
            StateError::InvalidArgument("No delivery is available for deploy.".to_string())
        })?;
    let delivery = store
    ...
    let phase = delivery
        ...
        .or_else(|| delivery.phases.last())
        .ok_or_else(|| StateError::StateCorrupted("Delivery has no phase.".to_string()))?;
    let architecture_ref = phase
        ...
        .ok_or_else(|| {
            StateError::InvalidArgument(
                "Deploy requires accepted AAC runtime_delivery. architectureArtifact ref is missing."
                    .to_string(),
            )
        })?;
...
fn deployment_shape(
    ...
    if let Some(value) = string_at(runtime, &["deploymentShape"]) {
        if value == "frontend-and-backend" {
            return DeploymentShape::FrontendAndBackend;
        }
        if value == "single-service" {
