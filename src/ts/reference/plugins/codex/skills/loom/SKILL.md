...
- `submit_existing_candidate`: read `instruction.requestRef`, use `requestReadPlan.groups[].readCommand` when request fields are needed, verify the named candidate/result files exist, then run `instruction.submitCommand`.
- `execute_task`: read `instruction.requestRef`, use `requestReadPlan.groups[].readCommand` to read required request fields, follow `executionRules.sourceEditPreparationContract` before source/artifact writes, execute only that TaskExecutionRequest against project files, write `instruction.resultFile`, then run `instruction.submitCommand`. A progress-only response before `resultFile` exists and submit succeeds is invalid.
...
- If `data.instruction.mode` is `execute_task`, read `data.instruction.requestRef`, use `requestReadPlan.groups[].readCommand` for required request fields, follow `executionRules.sourceEditPreparationContract` before source/artifact writes, complete the requested project-file task, write `data.instruction.resultFile`, and run `data.instruction.submitCommand`. Do not summarize or ask whether to continue after `next-task` or `record-result`; the task request has already been created and must be executed.
- If `data.instruction.mode` is `generate_candidate`, read `data.instruction.requestRef`, use `requestReadPlan.groups[].readCommand` for required request fields, and complete the candidate generation workflow before running any next loom routing command.
- If `data.instruction.mode` is `submit_existing_candidate`, read `data.instruction.requestRef`, use `requestReadPlan.groups[].readCommand` when request fields are needed, and submit the existing candidate/result files with `data.instruction.submitCommand` before running any next loom routing command.
...
- `next-task` returns a compact execution summary plus `data.instruction.mode = "execute_task"`. Always follow that instruction immediately: read `data.instruction.requestRef`, then use `requestReadPlan.groups[].readCommand` to inspect complete required fields before modifying files. Treat direct sourceRefs reads as fallback only when inspect or the request read plan is insufficient.
