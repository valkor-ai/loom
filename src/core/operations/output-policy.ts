...
export function refFirstRequestReadProtocol(): Record<string, unknown> {
  return {
    ...
    readRule: "After opening requestRef, use requestReadPlan.groups[].readCommand. If requestReadPlan is absent on an older request, use legacy agentAction.read.fieldGroups and each group's inspect readCommand.",
    nullFieldRule: "If a large field such as agentAction, sourceRefs, outputContract, rules, enumRefs, task, sourceContext, executionRules, or fieldAccessHints is null at the request root, read it through requestReadPlan/inspect first. Use listed *Ref files only as targeted fallback for the exact fields requested by the read group.",
    unlistedSidecarRule: "Do not probe unlisted .refs files such as section-schemas.json; listed refs are storage refs, not alternate contracts.",
  };
}
