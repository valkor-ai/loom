  ...
  return {
    ...
    clarificationConversationProtocol: {
      ...
      blockConfirmationRules: {
        ...
        final_summary: "Satisfied only after the user sees a combined final summary review, including the already-confirmed business scenario, decision impacts, lifecycle actions, key field/data elements, business-detail and page-operation-path confirmation when applicable or a concrete not-applicable reason when not applicable, and confirms it after the prior applicable blocks.",
      },
      ...
      requirementSemanticGrounding: {
        ...
        finalSummaryBusinessDetailContract: {
          ...
          requiredUserVisibleTopicsWhenApplicable: [
            ...
            "decision impact ordering",
            "business object lifecycle scan",
            "applicable objects or subjects",
            "applicable actions or behaviors",
            "key field/data elements grouped by identity, input, display, relationship, state, and result or feedback fields when applicable",
          ...
          finalSummaryReviewContract: {
            owningBlock: "final_summary",
            purpose: "Final summary is the user's last review surface before BrainstormCandidate submit. It must visibly review key field/data elements confirmed earlier instead of relying on hidden candidate retention.",
            rules: finalSummaryReviewRules(),
          },
    ...
    outputContract: {
      ...
      schemaShape: {
        ...
        candidateRules: [
          ...
          "Set finalSummaryConfirmed=true only after a dedicated final_summary block summarized scope, concepts, frontend target, nextPhasePreview, key field/data elements, and business-detail confirmation when applicable.",
          "If the current phase involves business flows, user operations, state changes, forms/fields, validation/blocking rules, or frontend/backend interaction, final_summary and candidate fields must preserve those details, including key field/data elements, using existing fields; do not leave them for PGC, AAC, TaskPlan, or TaskExecution to rediscover from the original requirement.",
