...
- `generate_knowledge_semantics`: read `instruction.requestRef` as `KnowledgeSemanticBuildRequest`; read every chunk body via each chunk's `readCommand`; write `instruction.resultFile` as `KnowledgeSemanticPackResult` with one result per chunk; run `instruction.submitCommand`; immediately follow any returned `generate_knowledge_semantics` instruction until the source is published or blocked. Do not ask whether to continue after `knowledge build` reports `semantic_pending`.
...
- If `data.instruction.mode` is `execute_task`, read `data.instruction.requestRef`, use `requestReadPlan.groups[].readCommand` when present or `agentAction.read.fieldGroups[].readCommand` as fallback for required request fields, follow `executionRules.sourceEditPreparationContract` before source/artifact writes, complete the requested project-file task, write `data.instruction.resultFile`, and run `data.instruction.submitCommand`. Do not summarize or ask whether to continue after `next-task` or `record-result`; the task request has already been created and must be executed.
- If `data.instruction.mode` is `generate_candidate`, read `data.instruction.requestRef`, use `requestReadPlan.groups[].readCommand` when present or `agentAction.read.fieldGroups[].readCommand` as fallback for required request fields, and complete the candidate generation workflow before running any next loom routing command.
- If `data.instruction.mode` is `submit_existing_candidate`, read `data.instruction.requestRef`, use `requestReadPlan.groups[].readCommand` when present or `agentAction.read.fieldGroups[].readCommand` as fallback when request fields are needed, and submit the existing candidate/result files with `data.instruction.submitCommand` before running any next loom routing command.
...
- `next-task` returns a compact execution summary plus `data.instruction.mode = "execute_task"`. Always follow that instruction immediately: read `data.instruction.requestRef`, then use `requestReadPlan.groups[].readCommand` when present or `agentAction.read.fieldGroups[].readCommand` as fallback to inspect complete required fields before modifying files. Treat direct sourceRefs reads as fallback only when inspect or the request read plan is insufficient.
...
- `agentAction.read.fieldGroups`: compatibility fallback for older requests without `requestReadPlan`.
...
Do not invent shell selectors or infer submit arguments from older artifacts when `requestReadPlan` or `agentAction` exists. If a field lookup returns `null`, return to the relevant read group, `referencedArtifactReadGuide`, and the named output contract fields instead of probing unrelated paths.
