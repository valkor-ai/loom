---
name: loom-deploy
description: Use when the user explicitly invokes @loom deploy or asks loom to prepare, run, inspect, log, stop, validate, bootstrap, or repair a local deployment through the Loom deploy protocol.
---

# loom deploy

This skill owns loom's explicit deploy mode for Codex. Keep the main `loom` skill focused on routing; keep deployment-specific workflow, diagnostics, active-operation handling, and repair boundaries here. This file is an adapter protocol, not the deploy engine design.

Every deploy workflow command must set:

```bash
LOOM_AGENT_PROFILE=codex LOOM_COMPACT_OUTPUT=1 "$HOME/.loom/bin/loom-cli" deploy ...
```
...
Use normal Codex file tools for source files, project docs, and small text artifacts. For `.loom` deployment JSON artifacts, repair request files, state files, logs, and generated candidates, use root `requestReadPlan.groups` inspect readCommands when present; otherwise use returned `agentAction.read.fieldGroups` inspect readCommands as compatibility fallback and read values from `data.fields[<field>].value`. If inspect is missing or fails, fall back to requestManifest refs and targeted selectors; direct full artifact reads are a correctness fallback only and must not be printed into chat.
...
`loom deploy repair` does not edit files by itself. Codex is the repair executor only inside the returned bounds.
...
- When the returned deploy repair or execution repair includes `requestRef`, read that request's root `requestReadPlan.groups` when present; otherwise use `agentAction.read.fieldGroups` as compatibility fallback and run the listed inspect readCommands before editing files. Use `data.fields[<field>].value` for returned values. If inspect fails, use the group fields through requestManifest refs and targeted selectors as the correctness fallback.
...
Keep `SKILL.md` as the workflow router. Deploy provider, stack, environment, bootstrap, and repair knowledge lives in this skill's `references/` directory and is part of the Codex deploy adapter package. Load only the focused reference needed for the returned deploy failure, selected stack, or repair action.
