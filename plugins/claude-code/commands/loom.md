...
After the MCP tool returns, load the installed Loom skill if needed and follow the action result. Continue immediately for `auto_runnable`, ask only for `user_gate`, and stop only for `done`, `blocked`, or `failed`.
