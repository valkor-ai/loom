...
- For a `user_gate` whose `gate.kind` is `vsefm_onboarding`, present the returned content and wait for the user's `required` or `deferred` choice. Then call `loom.verify` with that decision. Do not wait for an external V-SEFM result; Loom resumes immediately after recording the choice.
