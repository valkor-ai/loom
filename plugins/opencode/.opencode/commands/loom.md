...
- `generate_knowledge_semantics`: read `instruction.requestRef`; read chunks; copy `request.outputContract.resultTemplate`, fill `chunkResults[]`, write `instruction.resultFile`, submit, and follow semantic instructions until published/blocked. Do not ask after `semantic_pending`. Do not inspect Loom source files, dist, TS types, or old results to infer schema.
