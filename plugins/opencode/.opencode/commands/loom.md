...
- After a knowledge command returns, parse its JSON envelope; follow any returned CLI instruction or report the result compactly. Knowledge commands are not delivery requests, so do not route into phase planning, Brainstorm, candidate generation, task execution, review, repair, or deploy.
