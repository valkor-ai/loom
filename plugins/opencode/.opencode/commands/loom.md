...
For `active_operation`, call only the observation tools named by the result. For `repairable_error`, `stopAllowed=false`: repair only the returned file or target ids, follow `agentInstruction`, then call the returned resubmit tool.
