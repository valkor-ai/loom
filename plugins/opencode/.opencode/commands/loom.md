...
For `active_operation`, call only the observation tools named by the result. For a `user_gate` whose `gate.kind` is `vsefm_onboarding`, present the returned content, wait for `1` or `2`, then call `loom.verify` with `decision=required` for `1` or `decision=deferred` for `2` without waiting for an external result. For `repairable_error`, `stopAllowed=false`: inspect the returned request and read every required repair group first, then repair only the returned file or target ids, follow `agentInstruction`, and call the returned resubmit tool.
